...
class PT108061102{
    public:
        ...
        PT108061102 operator=(const PT108061102&);
};

class BD108061102{
    public:
        PT108061102 cells[ROW][COL];                       // The 5*6 board whose index (0,0) is start from the upper left CN
        void cell_reaction_marker();                // After the explosion, mark all the cell that  will explode in next iteration
        bool cell_is_full(PT108061102* cell);              // Check wether the cell is full of orbs and set the explosion variable to be true
        ...
        //const PT108061102* operator[] (const int) const;
       /* PT108061102* operator[] (const int);
        const PT108061102 * operator[] (const int &x) const {
            return cells[x];
        }*/
};

bool placement_illegal_ST108061102(Player player, PT108061102 cell){
...
PT108061102::PT108061102(){
    orbs_num = 0, capacity = 8, color='w', explode = false;
}

PT108061102::PT108061102(const PT108061102& P){
    orbs_num = P.orbs_num, capacity = P.capacity, color=P.color, explode = P.explode;
}

int PT108061102::get_capacity(){
    return this->capacity;
}

int PT108061102::get_orbs_num(){
    return this->orbs_num;
}

char PT108061102::get_color(){
    return this->color;
}

bool PT108061102::get_explode(){
    return this->explode;
}

void PT108061102::set_orbs_num(int orbs){
    this->orbs_num = orbs;
}

void PT108061102::set_capacity(int cap){
    this->capacity = cap;
}

void PT108061102::set_color(char col){
    this->color = col;
}

void PT108061102::set_explode(bool tof){
    this->explode = tof;
}

PT108061102 PT108061102::operator=(const PT108061102 &P) {
...
bool BD108061102::place_orb(int i, int j, Player * player){
    ...
    if(!placement_illegal_ST108061102(*player, cells[i][j])){
...
bool BD108061102::cell_is_full(PT108061102* cell){
...
int Score (int num, int last_num, int depth, Player player, char mycolor) {
    char rival = player.get_color() == 'r' ? 'b' : 'r';
    int Tscore;
    int CNx[4] = {0, 0, 5, 5}, CNy[4] = {0, 4, 0, 4};
    int EG1x[6] = {2, 4, 4, 2, 0, 0};
    int EG1y[6] = {0, 2, 3, 5, 2, 3};
    int EG2x[8] = {0, 0, 1, 1, 3, 3, 4, 4};
    int EG2y[8] = {2, 3, 0, 5, 0, 5, 2, 3};
    ...
    if (depth == 0) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (Tcolor == mycolor && TMPcolor != mycolor)
                sum -= 60;
            else if (Tcolor != mycolor && TMPcolor == mycolor) sum += 100;
        }
        for (int i = 0; i < 6; i++) {
            ...
            if (Tcolor == mycolor && TMPcolor != mycolor)
                sum -= 50;
            else if (Tcolor != mycolor && TMPcolor == mycolor) sum += 50;
        }
        ...
        for (int i = 0; i < 4; i++) {
            char Tcolor = CUR108061102.cells[CNy[i]][CNx[i]].get_color();
            char TMPcolor = TMP108061102[depth][num].cells[CNy[i]][CNx[i]].get_color();
            if (Tcolor == rival && TMPcolor != rival)
                sum += 150;
            else if (Tcolor != rival && TMPcolor == rival) sum -= 150;
        }
        for (int i = 0; i < 6; i++) {
            char Tcolor = CUR108061102.cells[EG1x[i]][EG1y[i]].get_color();
            char TMPcolor = TMP108061102[depth][num].cells[EG1x[i]][EG1y[i]].get_color();
            if (Tcolor == rival && TMPcolor != rival)
                sum += 100;
            else if (Tcolor != rival && TMPcolor == rival) sum -= 100;
        }
        ...
        for (int i = 0; i < ROW; i++) {
            for (int j = 0; j < COL; j++) {
                if (TMP108061102[depth][num].cells[i][j].get_color() == rival)
                    sum -= 2;
                else WINc++;
            }
        }
        ...
        for (int i = 0; i < ROW; i++) {
            for (int j = 0; j < COL; j++) {
                if (last_num == -1)
                    ...
                    TMP108061102[depth][num] = TMP108061102[depth + 1][last_num];
                if (TMP108061102[depth][num].get_cell_color(i, j) != rival) {
...
void algorithm_A(Board board, Player player, int index[]){
    ...
    for (int i = 0; i < ROW; i++) {
        for (int j = 0; j < COL; j++) {
            ...
            if (board.get_cell_color(i, j) != rival) {
                y108061102 = i;
                x108061102 = j;
                y = i;
                x = j;
