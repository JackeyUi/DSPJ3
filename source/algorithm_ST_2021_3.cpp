...
/*PT102* BD108061102::operator[](int c) {
    return cells[c];
}*/
...
int x108061102, y108061102, Tscore;
BD108061102 TMP[5][50], CUR108061102, BestScore;
...
int Score (int num, int last_num, int depth, Player player, char mycolor) {
    ...
    if (depth == 0) {
        int sum = 0;
        TMP[depth][num] = TMP[depth + 1][last_num];
        for (int i = 0; i < 4; i++) {
            char Tcolor = CUR108061102.cells[CNy[i]][CNx[i]].get_color();
            char TMPcolor = TMP[depth][num].cells[CNy[i]][CNx[i]].get_color();
            if (Tcolor == mycolor && TMPcolor != mycolor)
                sum -= 60;
            else if (Tcolor != mycolor && TMPcolor == mycolor) sum += 90;
        }
        for (int i = 0; i < 6; i++) {
            char Tcolor = CUR108061102.cells[EG1x[i]][EG1y[i]].get_color();
            char TMPcolor = TMP[depth][num].cells[EG1x[i]][EG1y[i]].get_color();
            if (Tcolor == mycolor && TMPcolor != mycolor)
                sum -= 40;
        ...
        for (int i = 0; i < 8; i++) {
            ...
            char TMPcolor = TMP[depth][num].cells[EG2x[i]][EG2y[i]].get_color();
            if (Tcolor == mycolor && TMPcolor != mycolor)
                sum -= 40;
            else if (Tcolor != mycolor && TMPcolor == mycolor) sum += 40;
        }
        ...
        for (int i = 0; i < ROW; i++) {
            for (int j = 0; j < COL; j++) {
                if (TMP[depth][num].cells[i][j].get_color() == mycolor) {
                    sum+=10;
                }
            }
        }
        ...
        for (int i = 0; i < ROW; i++) {
            for (int j = 0; j < COL; j++) {
                if (TMP[depth][num].cells[i][j].get_color() == invert) {
                    sum -= 2;
                }
                else if(TMP[depth][num].cells[i][j].get_color() == mycolor) {
        ...
        if (sum >= Tscore) BestScore = TMP[depth][num];
        return sum;
    }
    ...
    if (player.get_color() == mycolor) {
        ...
        for (int i = 0; i < ROW; i++) {
            for (int j = 0; j < COL; j++) {
                if (last_num == -1) {
                    TMP[depth][num] = CUR108061102;
                }
                else {
                    TMP[depth][num] = TMP[depth + 1][last_num];
                }
                if (TMP[depth][num].get_cell_color(i, j) != invert) {
                    TMP[depth][num].place_orb(i, j, &player);
